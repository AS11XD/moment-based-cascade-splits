...
void CDDSImage::create_textureCubemap(unsigned int format, unsigned int components, const CTexture& positiveX, const CTexture& negativeX,
    ...
    if(format != 0);
    if(components != 0);
    if(positiveX.get_depth() == 1);

    // verify that all dimensions are the same
    if(same_size(positiveX, negativeX));
    if(same_size(positiveX, positiveY));
    if(same_size(positiveX, negativeY));
    if(same_size(positiveX, positiveZ));
    if(same_size(positiveX, negativeZ));
...
// flipImage - specifies whether image is flipped on load, default is true
void CDDSImage::load(const string& filename, istream& is, bool flipImage) {
    ...
    if (strncmp(filecode, "DDS ", 4) != 0) {
        throw runtime_error(filename + " not a DDS file");
    }
    ...
    if (ddsh.ddspf.dwFlags & DDSF_FOURCC) {
        ...
        case FOURCC_DXT5:
			...
			break;
        default:
            throw runtime_error(filename + " unknown texture compression '" + fourcc(ddsh.ddspf.dwFourCC) + "'");
        }
    }
        ...
        throw runtime_error(filename + " unknow texture format");
    }
...
void CDDSImage::write_texture(const CTexture& texture, ostream& os) {
    if(get_num_mipmaps() == texture.get_num_mipmaps());
...
void CDDSImage::upload_texture1D() {
    ...
    if(baseImage.get_height() == 1);
    if(baseImage.get_width() > 0);
...
void CDDSImage::upload_texture2D(uint32_t imageIndex, uint32_t target) {
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_LEVEL, image.get_num_mipmaps() - 1); // opengl likes array length of mipmaps
