...
ISlider::ISlider(int* ad, std::string n, float textlength, float charSizeX, int minV, int maxV, float x, float y, float sx, float sy, float r, float g, float b, float a)
	: MenuItem<int>(ad, x, y, sx, sy, r, g, b, a)
{
...
void ISlider::renderItem(MenuController* menuController, TextRenderer* tR, float offsetY, float discardAtYBottom, float discardAtYTop, glm::mat4x4& MVP)
{
	if (!menuController->getMenuInvisible()) {
		...
		if (name.compare("") != 0) {
			tR->RenderText(std::to_string(*adress), posX + sizeX * 0.75f, posY + offsetY + sizeY * 0.1f, csx, sizeY * 0.8f, cr, cg, cb, ca, MVP, true, discardAtYBottom, discardAtYTop);
		}
	}
}

void ISlider::input(int w, int h, float offsetY, bool* holder)
{
	int mx, my;
	MouseXY(&mx, &my);
	my = h - my;
	...
	if (*holder && myHold) {
		...
		inne *= (max - min);
		inne += min;
		*adress = std::floor(std::min(std::max(inne, min), max));
	}
}